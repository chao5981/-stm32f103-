...
void Calibrate_or_Get_TouchParaWithFlash(uint8_t LCD_Mode,uint8_t forceCal)
{
	...
	//读取触摸参数标志
	Flash_SPI_BufferRead(&para_flag,FLASH_TOUCH_PARA_FLAG_ADDR,1);
	...
	{ 		
		...
		if(  para_flag == FLASH_TOUCH_PARA_FLAG_VALUE && forceCal == 1)
		{
			Flash_SPI_BufferRead((uint8_t *)&strXPT2046_TouchPara,FLASH_TOUCH_PARA_ADDR,4*6*8);	
		}
		...
		//写入触摸参数标志
		Flash_SPI_BufferWrite(&para_flag,FLASH_TOUCH_PARA_FLAG_ADDR,1);
		//写入最新的触摸参数
		Flash_SPI_BufferWrite((uint8_t *)&strXPT2046_TouchPara,FLASH_TOUCH_PARA_ADDR,4*6*8);
