...
3.调用RCC_GetClocksFreq函数，然后打印即可。打印格式如下：
 ...
 相比于标准库，HAL库提供了很多方便的函数。例如:返回sysclk,hclk,pclk1,pclk2的时钟频率;启动时钟安全系统,以及对应的回调函数,还有延时函数.

 解释一下时钟安全系统HAL_RCC_EnableCSS (void):如果检测到高速外部（HSE）振荡器时钟发生故障，该振荡器会被自动禁用，同时会产生一个中断来通知软件发生了故障（时钟安全系统中断，CSSI），使微控制器（MCU）能够执行恢复操作。CSSI 与 Cortex-M3 的不可屏蔽中断（NMI）异常向量相关联。
