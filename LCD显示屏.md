...
![image](https://github.com/user-attachments/assets/4daa696e-29c1-4daf-b580-873c2fdf96dd)
  ...
  哦对，这里的LCD显示图像是一件麻烦的事情，因为它毕竟不是点阵屏，点阵屏只需要把相对应的引脚拉高拉低就好了，但这里不是，现在我能想到的就一种方法：设计一个程序，让其可以在上面描点画图，最后画完图后导出每个点的坐标以俩个数组存放，最后用函数ILI9341_SetPointPixel(uint16_t usX, uint16_t usY)输入坐标填充相应的颜色即可。
