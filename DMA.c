...
void ADCx_DMA_Configuration(void)
{
	...
	//要传输的数据数量
	ADCx_DMA_Sturcture.DMA_BufferSize=NOFCHANEL;
	...
	//寄存器是否每传输一项自动增加"存储器地址"
	ADCx_DMA_Sturcture.DMA_MemoryInc=DMA_MemoryInc_Enable;
	
	//DMA模式选择，_Normal:一次传输就停止;Circular:自动重启
	ADCx_DMA_Sturcture.DMA_Mode=DMA_Mode_Circular;
	
	//外设的地址，注意，DMA_PeripheralBaseAdd的类型是uint32_t，而&ADCx->DR是指针,所以需要强转
