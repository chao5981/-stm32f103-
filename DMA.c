...
void ADCx_DMA_Configuration(void)
{
	...
	//复位DMA
	DMA_DeInit(ADCx_1_DMA_Channel);
	
	//要传输的数据数量
	ADCx_DMA_Sturcture.DMA_BufferSize=ADCx_1_DMA_SIZE;
	...
	//目标端的每项数据的大小
	ADCx_DMA_Sturcture.DMA_MemoryDataSize=DMA_MemoryDataSize_Word;
	...
	ADCx_DMA_Sturcture.DMA_PeripheralBaseAddr=( uint32_t ) ( & ( ADCx_1->DR ) );
	
	
	//源头端的每项数据的大小
	ADCx_DMA_Sturcture.DMA_PeripheralDataSize=DMA_PeripheralDataSize_Word;
	...
	//初始化结构体
	DMA_Init(ADCx_1_DMA_Channel, &ADCx_DMA_Sturcture);
	
	/// 使能 DMA 通道
	DMA_Cmd(ADCx_1_DMA_Channel,ENABLE);
}
